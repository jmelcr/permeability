...
for n_unsat in `seq 0 12 $n_lipids_tot` $n_lipids_tot 
do
...
for init_conf in LOrdered
do
mkdir -p sim1_awh_${init_conf} && cd sim1_awh_${init_conf} || exit 11
tsp gmx grompp -c ${rd}/../prep_min_eq/long_eq/confout.gro  -f ${rd}/../../${mdp_fname} -n ${rd}/index.ndx -p ../system.top -maxwarn 1
